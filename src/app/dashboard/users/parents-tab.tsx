...
// live on the 学生 tab now (kind:'parent' filter), fixing the bug where a pure-student account leaked in
// here. Defence-in-depth: even though the page hides this tab for non-managers, we re-check the permission
...
export default async function ParentsTab() {
  ...
  const [users, students] = await Promise.all([
    listPortalUsers(ctx, { kind: 'parent' }),
    listStudents(),
  ])
                ...
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    <span className="text-sm font-medium">
                      {u.name}{' '}
                      <span className="text-xs font-normal text-neutral-500">{u.role}</span>
                    </span>
